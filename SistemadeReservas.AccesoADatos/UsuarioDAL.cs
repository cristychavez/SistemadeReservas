...
using System.Linq;
using System.Segurity.Cryptograpy;
using System.Text;
...
namespace SistemadeReservas.AccesoADatos
{
    public class UsuarioDAL
    {
        ...
        public static async Task<bool> ExisteLogin(Usuario pUsuario, BDContexto pDbContext)
        {
            bool result = false; 
        ...
        public static async Taks<int> EliminarAsync(Usuario pUsurio)
        {
            int result = 0;
            using (var bdContexto = new BDContexto())
            {
                var usuario = await bdContexto.Usuario.FirstOrDefaultAsync(s => s.Id == pUsurio.Id);
                bdContexto.Usurio.Remove(usuario);
                result = await bdContexto.SaveChangesAsync();
            }
            return result;

        }
        public static async Task<Usuario> ObtenerPorAsync(UsuarioDAL pUsuario)
        {
        ...
        internal static IQueryable<Usuario> QuerySelect(IQueryable<usuario> pQuery, UsuarioDAL pUsuario)
        {
            if (pUsuario.Id > 0)
            ...
            if (!string.IsNullWhiteSpace(pUsuario.Login))
            ...
            {
                DateTime fechaInicial = new DateTime(pUsuario.FechaRegistro.Year, pUsuario.FechaRegistro.Month, pUsuario.FechaRegistro.Day, 0);
        ...
        public static async Task<List<Usuario>> BuscarIncluirRolesAsync(Usuario pUsuario)
        {
            ...
            {
                var select = bdContexto.Usuario.AsQueryable();
                select = QuerySelect(select, pUsuario).Include(select => s.Rol).AsQueryable();
                usuarios = await select.ToListAsync();
            }
            return usuarios;
        }
        public static async Task<Usuario> LoginAsync(Usuario pUsuario)
        {
            ...
            {
                EncriptarMD5(pUsuario);
                usuario = await bdContexto.Usuario.FirstOrDefaultAsync(s => s.Login == pUsuario.Login && s.Password
                == pUsuario.Password && s.Estatus == (byte)Estatus_Usuario.ACTIVO);
            }
            return usuario;
        }
